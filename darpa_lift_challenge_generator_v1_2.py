...
"""
DBbun Synthetic Mission and Aircraft Generator (v1.2)
-------------------------------------------------------
...
CONFIG: Dict[str, Any] = {
    ...
    # Output goes to D: (external drive) instead of C: so large runs never threaten your
    # system drive's free space. Change the drive letter/folder below if yours differs.
    "OUTPUT_DIR": r"D:\Temp\output",

    # ALL per-second telemetry for ALL missions is recorded — nothing is sampled or
    # dropped. Memory stays bounded by writing the timeseries CSV incrementally in
    # batches (every FLUSH_EVERY_N_DESIGNS designs) instead of holding the entire run's
    # data in RAM until the end. This is what actually caused the earlier RAM/pagefile
    # exhaustion at N=10000 — not the final file size.
    "FLUSH_EVERY_N_DESIGNS": 50,
    "PROGRESS_EVERY_N_DESIGNS": 100,   # print a heartbeat every N designs
    ...
    # Competition metadata (from DARPA PM Philip Smith, VFS 2026)
    # FAA regulatory framework (confirmed: Brandon Roberts & Marcus Cunningham, DARPA Lift Video 3)
    "FAA_REGULATORY_BASIS":  "Part 107 + Section 44807 umbrella exemption",
    "FAA_PROPULSION_RESTRICTION": "None — Part 107 is performance-based and propulsion-agnostic",
    "REMOTE_ID_MODULE_MASS_KG": 0.10,  # broadcast module ~few ounces; required on all competition aircraft
    "COMPETITION_DATE":      "2026-08-02 to 2026-08-09",
    "COMPETITION_VENUE":     "National Museum of the US Air Force, Dayton, Ohio",
    "COMPETITION_PRIZE_POOL_USD": 6_500_000,
    "PRIZE_FULL_THRESHOLD_RATIO": 4.0,   # 4:1 = full prize; below = 50% of prize
    "HOVER_TURN_COUNT_RANGE": (16, 20),  # per Q&A: hover turns are critical design factor
    "HOVER_TURN_DURATION_S":  12.0,      # seconds at near-hover power per turn
    "HOVER_TURN_POWER_MULT":  0.85,      # fraction of hover power during a turn (decel/accel)
    ...
    "CRUISE_ALTITUDE_TOL_FT": 50.0,  # tolerance band for altitude hold reporting (reserved for future rule checks)
    ...
    "PAYLOAD_MASS_KG_RANGE": (50.0, 100.0),  # extended to 100 kg to allow 4:1 ratio designs
    ...
    "GLIDE_POWER_REDUCTION_BASE": 0.10,   # base cruise power reduction for glide-segment planning
    "GLIDE_WIND_BONUS_MAX": 0.05,         # additional reduction in strong winds (dynamic soaring proxy)
    "WIND_POWER_PENALTY_FACTOR": 0.12,   # reduced: wind already penalises groundspeed; double-counting at 0.25 was too aggressive
    "TURB_POWER_PENALTY_FACTOR": 0.18,  # reduced: turbulence adds ~10-20% extra hover power in practice, not 50-75%
    ...
    "BATTERY_MAX_C_RATE_RANGE": (5.0, 14.0),     # peak discharge in C (Wh-based approximation) — floor raised so minimum battery can sustain hover
...
PROPULSION_ARCHITECTURES: Dict[str, Dict[str, Any]] = {
    ...
    "pure_gas_multirotor": {
        # FAA Video 3: Part 107 is propulsion-agnostic; gas/ICE explicitly legal.
        # Philip Smith VFS speech: "lightweight power-dense ICE from UAV/hobbyist world"
        # as a key available technology.
        # Physics: ICE ~3,000–4,500 Wh/kg effective (fuel × 30% efficiency),
        # heavier motor drivetrain but enormous energy reserve vs battery.
        "prob": 0.08,
        "primary_propulsor_type": "gas_multirotor",
        "secondary_propulsor_type": "none",
        "secondary_propulsor_fraction_range": (0.0, 0.0),
        "hover_power_factor": 1.05,    # heavier drivetrain vs electric
        "cruise_power_factor": 0.90,
        "extra_failure_risk": 0.04,    # vibration, fuel system, starting
        "energy_spec_range_Wh_per_kg": (3000.0, 4500.0),   # effective fuel energy density
        "battery_c_rate_multiplier": 0.25,  # slow power delivery from fuel (vs fast battery)
    },
    "series_hybrid_gas_electric": {
        # Gas generator (constant speed, high efficiency) → electricity → electric drive.
        # Best of both: electric motor responsiveness + fuel energy density.
        # Philip Smith: "hybridization, combining the best of both worlds."
        "prob": 0.07,
        "primary_propulsor_type": "series_hybrid",
        "secondary_propulsor_type": "none",
        "secondary_propulsor_fraction_range": (0.0, 0.0),
        "hover_power_factor": 0.95,    # electric drive efficient; generator buffers peaks
        "cruise_power_factor": 0.80,   # generator at optimum speed → high cruise efficiency
        "extra_failure_risk": 0.05,    # complex: generator + inverter + buffer battery + motors
        "energy_spec_range_Wh_per_kg": (2000.0, 3500.0),   # fuel + buffer battery blend
        "battery_c_rate_multiplier": 0.50,  # buffer battery + generator headroom
    },
}
...
class MissionResult:
    ...
    is_4to1_qualifying: bool          # True if success + empty≤55lb + payload≥220lb
    prize_tier: str                   # "none" / "partial" / "full" / candidate for subjective
...
def generate_design(rng: random.Random) -> AircraftDesign:
    ...
    # Remote ID broadcast module: required on all competition aircraft (FAA Part 89).
    # Broadcast modules weigh "a few ounces" (Brandon Roberts / Marcus Cunningham, DARPA Lift Video 3).
    empty_mass_kg += cfg.get("REMOTE_ID_MODULE_MASS_KG", 0.10)
    ...
    # Gas and hybrid propulsion architectures sample energy early so they can override spec energy.
    # We choose architecture here to resolve the energy override before battery_spec_energy is set.
    arch_key_pre, arch_prof_pre = choose_propulsion_architecture(rng)
    if "energy_spec_range_Wh_per_kg" in arch_prof_pre:
        # Gas/hybrid: use fuel-based effective energy density instead of battery chemistry
        spec_min, spec_max = arch_prof_pre["energy_spec_range_Wh_per_kg"]
        energy_type = "gas_or_hybrid"   # label for reporting
    ...
    # Gas/hybrid architectures have slower effective power delivery than batteries
    if "battery_c_rate_multiplier" in arch_prof_pre:
        c_rate *= arch_prof_pre["battery_c_rate_multiplier"]
    ...
    # Use the architecture already chosen above (to allow energy override)
    arch_key, arch_prof = arch_key_pre, arch_prof_pre
...
def simulate_mission(design: AircraftDesign, env: Environment, rng: random.Random) -> Tuple[MissionResult, List[TimeStep]]:
    ...
    def cruise_power_required_W(mass_kg: float, speed_mps: float, wind_frac: float, turb_frac: float, wing_deployed: bool) -> float:
        ...
        if "MISSION_GLIDE_SEGMENTS" in design.traits:
            # Base 10% reduction from glide-segment planning; additional wind-dependent
            # bonus models dynamic soaring energy extraction (scales with wind gradient).
            # Standalone: 10-15% depending on wind. Stacked with WING_HIGH_ASPECT: 17-22%,
            # consistent with the lower bound of albatross dynamic-soaring literature.
            glide_base = cfg["GLIDE_POWER_REDUCTION_BASE"]
            wind_bonus = cfg["GLIDE_WIND_BONUS_MAX"] * clamp(wind_frac, 0.0, 1.0)
            base *= (1.0 - glide_base - wind_bonus)

        if design.wing_foldable and wing_deployed:
            base *= 0.94

        if "WING_MORPHING_MEMBRANE" in design.traits:
            eff_gain = 1.0 - 0.0667 * clamp(0.6 * wind_frac + 0.8 * turb_frac, 0.0, 1.5)  # max reduction 10% at peak env
    ...
    def apply_env_penalties(power_W: float, wind_frac: float, turb_frac: float) -> float:
        ...
        if "STAB_GUST_REJECTION" in design.traits:
            # Reduce only the environmental penalty portion, not the baseline power.
            # At zero wind/turbulence the penalty is zero so the modifier has no effect.
            env_penalty = factor - 1.0
            factor = 1.0 + 0.97 * env_penalty
        return power_W * factor

    def available_power_W(temperature_C: float, supercap_energy_Wh: float, in_burst: bool = False) -> float:
        ...
        # Burst boosts battery output only; supercap already provides its own peak power
        # independently. Multiplying (p_batt + p_sc) by f_burst would double-count
        # the supercap's burst contribution.
        if in_burst and design.burst_power_factor > 1.0:
            p_total = max(p_batt * design.burst_power_factor + p_sc, 0.0)
        else:
            p_total = max(p_batt + p_sc, 0.0)
    ...
    # ---- Hover turn state -----------------------------------------------
    # Hover turns are circuit waypoints where the aircraft decelerates to
    # near-hover, reorients, then accelerates. Each costs ~hover_power for
    # hover_turn_duration_s seconds.  (DARPA PM Q&A: 16-20 turns critical.)
    _n_turns = rng.randint(*cfg["HOVER_TURN_COUNT_RANGE"])
    _turn_duration = cfg["HOVER_TURN_DURATION_S"]
    _turn_power_mult = cfg["HOVER_TURN_POWER_MULT"]
    # Distribute turns evenly across the loaded leg
    _loaded_total = nm_to_m(cfg["PAYLOAD_LEG_DISTANCE_NM"])
    _turn_spacing = _loaded_total / max(_n_turns, 1)
    _next_turn_dist = _turn_spacing          # distance along leg to next turn
    _in_turn = False
    _turn_elapsed = 0.0
    ...
    while True:
        ...
        if emergency_active:
            ...
            if phase == "takeoff_climb":
                ...
                # NOTE: burst_power_factor boosts AVAILABLE power (via supercap/motor headroom),
                # not demand. Demand is set by physics; burst means the aircraft CAN supply more.

            elif phase == "loaded_cruise":
                speed_mps = cruise_groundspeed(commanded_cruise_speed, wind_frac, turb_frac)
                vertical_rate = 0.0

                # Hover turn: decelerate to near-hover at each waypoint
                leg_dist = distance_m - seg_start_distance_m
                if not _in_turn and leg_dist >= _next_turn_dist and _next_turn_dist < _loaded_total:
                    _in_turn = True
                    _turn_elapsed = 0.0
                if _in_turn:
                    _turn_elapsed += dt
                    # During the turn the aircraft hovers — speed drops to near zero
                    speed_mps = max(speed_mps * 0.15, 0.0)
                    p_req = apply_env_penalties(
                        hover_power_required_W(mass_kg) * _turn_power_mult,
                        wind_frac, turb_frac
                    )
                    if _turn_elapsed >= _turn_duration:
                        _in_turn = False
                        _next_turn_dist += _turn_spacing   # advance to next turn
                else:
                    p_req = apply_env_penalties(
                        cruise_power_required_W(mass_kg, speed_mps, wind_frac, turb_frac, wing_deployed),
                        wind_frac, turb_frac
                    )
        ...
        in_burst = (
            phase == "takeoff_climb"
            and design.burst_power_factor > 1.0
            and time_takeoff_climb_actual < design.burst_duration_s
            and not emergency_active
        )
        p_av = available_power_W(temperature_C, supercap_energy_Wh, in_burst=in_burst)
    ...
    # 4:1 ratio qualifying (full prize threshold per DARPA PM Philip Smith, VFS 2026)
    _4to1_threshold = cfg.get("PRIZE_FULL_THRESHOLD_RATIO", 4.0)
    is_4to1_qualifying = (
        is_qualifying_run and
        design.payload_mass_kg / max(design.empty_mass_kg, 1e-6) >= _4to1_threshold
    )

    # Prize tier: none → partial (≥2:1) → full (≥4:1)
    if not is_qualifying_run:
        prize_tier = "none"
    elif is_4to1_qualifying:
        prize_tier = "full"
    else:
        prize_tier = "partial"   # qualifies but below 4:1 threshold → 50% prize
    ...
    mission = MissionResult(
        ...
        is_4to1_qualifying=is_4to1_qualifying,
...
def generate_universe(
...
) -> Tuple[List[AircraftDesign], List[MissionResult], int]:
    """
    Generates the full design/mission/telemetry universe. ALL per-second telemetry
    for ALL missions is recorded — nothing is sampled or capped.

    To keep memory bounded at any N_DESIGNS, telemetry is written to ts_path
    INCREMENTALLY in batches of FLUSH_EVERY_N_DESIGNS designs, rather than being
    held in one giant list for the entire run. This is the actual fix for the
    RAM/pagefile exhaustion seen at N_DESIGNS=10000 — the old code held every
    row in memory until the very end before writing anything.

    Returns (designs, missions, total_ts_rows_written). The timeseries DataFrame
    itself is not returned (it has already been written to disk) to avoid
    re-accumulating it in memory.
    """
    cfg = CONFIG
    designs: List[AircraftDesign] = []
    missions: List[MissionResult] = []

    flush_every = max(1, int(cfg.get("FLUSH_EVERY_N_DESIGNS", 50)))
    progress_every = max(1, int(cfg.get("PROGRESS_EVERY_N_DESIGNS", 100)))

    ts_buffer: List[TimeStep] = []
    total_ts_rows = 0
    header_written = False

    def flush_ts_buffer():
        nonlocal ts_buffer, header_written, total_ts_rows
        if not ts_buffer or ts_path is None:
            ts_buffer = []
            return
        df_chunk = pd.DataFrame([asdict(x) for x in ts_buffer])
        df_chunk.to_csv(ts_path, mode="a", header=(not header_written), index=False)
        header_written = True
        total_ts_rows += len(ts_buffer)
        ts_buffer = []

    # Start with a clean file (so re-running doesn't append to stale old data)
    ...
    for i in range(n_designs):
        ...
        for _ in range(missions_per_design):
            ...
            ts_buffer.extend(ts)   # ALL rows kept — nothing sampled

        if (i + 1) % flush_every == 0:
            flush_ts_buffer()

        if (i + 1) % progress_every == 0 or (i + 1) == n_designs:
            elapsed = time.time() - t_start
            rate = (i + 1) / elapsed if elapsed > 0 else 0.0
            eta_s = (n_designs - (i + 1)) / rate if rate > 0 else 0.0
            rows_so_far = total_ts_rows + len(ts_buffer)
            print(f"[{cfg['VERSION']}] {i+1:,}/{n_designs:,} designs "
                  f"({len(missions):,} missions, {rows_so_far:,} telemetry rows written so far) — "
                  f"{elapsed:.0f}s elapsed, ~{eta_s:.0f}s remaining")

    flush_ts_buffer()   # final partial batch

    compute_design_rankings(designs, missions)
    return designs, missions, total_ts_rows
...
if __name__ == "__main__":
    ...
    os.makedirs(out_dir, exist_ok=True)
    print(f"[{CONFIG['VERSION']}] Output directory: {out_dir}")
    print(f"[{CONFIG['VERSION']}] Recording ALL telemetry for ALL missions "
          f"(streamed to disk in batches of {CONFIG.get('FLUSH_EVERY_N_DESIGNS', 50)} "
          f"designs to keep memory bounded — nothing is sampled).")
    ...
    designs, missions, total_ts_rows = generate_universe(
        CONFIG["N_DESIGNS"], CONFIG["MISSIONS_PER_DESIGN"], rng, ts_path=ts_path
    )
    ...
    # Note: missions_timeseries.csv was already written incrementally during generation.
    ...
    print(f"[{CONFIG['VERSION']}] Generated {total_ts_rows:,} time-series rows (100% of all "
          f"missions, nothing sampled) -> {ts_path}")

    if bool(CONFIG.get("MAKE_PLOTS", True)):
        # Plotting reads the full timeseries back from disk rather than holding it in
        # memory the whole run — only loaded if MAKE_PLOTS is actually requested.
        df_ts_for_plots = pd.read_csv(ts_path)
        plots_dir = os.path.join(out_dir, str(CONFIG.get("PLOTS_DIR", "plots")))
        generate_plots(df_designs, df_missions, df_ts_for_plots, plots_dir)
