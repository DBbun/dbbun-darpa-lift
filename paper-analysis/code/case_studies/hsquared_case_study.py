"""
H-Squared case study: real single-main-rotor + tail-rotor helicopter (same
architecture gap as AVIDrone/Xtreme Aerial -- forced to rotor_count=4,
multirotor). Cruise speed is NOT guessed here -- it's read directly off the
competition telemetry overlay (30.9 mph = 13.8 m/s).
...
EMPTY_MASS_KG = 24.81    # 54.7 lb
PAYLOAD_MASS_KG = 73.44  # 161.9 lb
ROTOR_COUNT = 4          # forced -- real is single main rotor + tail rotor
CRUISE_SPEED_MPS = 13.8  # 30.9 mph, read directly from telemetry overlay

battery_mass_kg = 5.0
battery_spec_energy = 210.0
battery_energy_Wh = battery_mass_kg * battery_spec_energy
battery_voltage_V = 48.0
battery_max_power_W = 10500.0
...
hsquared = gen.AircraftDesign(
    design_id="DLIFT_HSQUARED_CASE",
    ...
    cruise_speed_mps=CRUISE_SPEED_MPS, climb_rate_mps=3.5, mode_count=2,
    ...
    design_summary="H-Squared case study (real-world reconstruction).",
...
print("=== H-Squared case-study design ===")
print(f"empty_mass_kg={EMPTY_MASS_KG}  payload_mass_kg={PAYLOAD_MASS_KG:.2f}  ratio={ratio:.3f}")
print(f"cruise_speed_mps={CRUISE_SPEED_MPS} (measured, not guessed)")
...
for _ in range(N_MISSIONS):
    ...
    mission, _ts = gen.simulate_mission(hsquared, env, rng)
