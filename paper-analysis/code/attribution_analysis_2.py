"""
Attribution analysis round 2: the first round showed the variance driver
is NOT touchdown/climb-rate, it's somewhere in the "everything else"
bucket. This splits that bucket into 3 mechanistically distinct groups to
find the actual driver:

  C) power_margin_efficiency: power_margin, motor_eff, esc_eff -- governs
     whether INSTANTANEOUS power demand can be met (power_saturation risk).
  D) battery_capacity: battery_mass_kg, battery_spec_energy -- governs
     total ENERGY capacity / endurance (energy_depleted risk).
  E) structural_soft: gust_rejection_gain, frame_stiffness,
     landing_gear_mass_kg, tendon_cable_fraction, cruise_speed_mps,
     battery_voltage_V -- the remaining softer/cosmetic-ish fields.

Same 3 teams (Xtreme Aerial, H-Squared, MacGyver), same fixed point
estimates for whatever isn't varied in a given condition.
...
def make_design(design_id, empty_mass_kg, payload_mass_kg, rotor_count, fixed_cruise, vals):
    ...
    return gen.AircraftDesign(
        ...
        design_summary="attribution analysis round 2 draw.",
...
def rank_score_and_reasons(design, n_missions, rng):
    results = []
    ...
    n_power_sat = sum(1 for m in results if m.failure_reason == "power_saturation")
    n_energy_dep = sum(1 for m in results if m.failure_reason == "energy_depleted")
    return rank_score, n_power_sat / n, n_energy_dep / n


master_rng = random.Random(20260810)

print(f"{'Team':>15} {'Condition':>22} {'mean':>7} {'median':>8} {'stdev':>7} {'min':>6} {'max':>6} {'pow_sat%':>9} {'energy_dep%':>12}")

for (name, empty_kg, payload_kg, rotor_count, fixed_cruise) in TEAMS:
    for condition in ("power_margin_efficiency", "battery_capacity", "structural_soft"):
        scores = []
        pow_sats = []
        energy_deps = []
        for draw_i in range(N_DRAWS):
            ...
            vals = dict(FIXED)
            if condition == "power_margin_efficiency":
            ...
            d = make_design(f"DLIFT_ATTR2_{name}_{condition}_{draw_i}", empty_kg, payload_kg, rotor_count, fixed_cruise, vals)
            score, pow_sat_rate, energy_dep_rate = rank_score_and_reasons(d, N_MISSIONS_PER_DRAW, rng)
            scores.append(score)
            pow_sats.append(pow_sat_rate)
            energy_deps.append(energy_dep_rate)

        print(f"{name:>15} {condition:>22} {statistics.mean(scores):7.3f} {statistics.median(scores):8.3f} "
              f"{statistics.stdev(scores):7.3f} {min(scores):6.3f} {max(scores):6.3f} "
              f"{100*statistics.mean(pow_sats):8.2f}% {100*statistics.mean(energy_deps):11.2f}%")
